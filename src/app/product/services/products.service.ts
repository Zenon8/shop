...
export class ProductsService {
  ...
  constructor(private config: ConfigOptionsService) {
    ...
    this.productList = [
      new ProductModel(1, 'HP 250 G6', 500, [new ImageModel(hpImg, width, height)], 10, Category.NOTEBOOK),
      new ProductModel(2, 'Lenovo IdeaPad 320-15IKB ', 400, [new ImageModel(lenovoImg, width, height)], 5, Category.NOTEBOOK),
      new ProductModel(3, 'Samsung Galaxy Note 9', 1250, [new ImageModel(samsungImg, width, height)], 10, Category.PHONE),
      new ProductModel(4, 'Asus ZenPad 8.0', 250, [new ImageModel(asusImg, width, height)], 12, Category.TABLET)
    ];
  }
  ...
  getImageSize(el: object) {
    let key = '';
    if (el instanceof ProductCardComponent) {
